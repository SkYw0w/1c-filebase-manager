...
Процедура ВыполнитьСкрипт()
    ...
    Попытка
        ...
        // Читаем файл (ibases.v8i обычно в UTF-8, но могут быть варианты)
        Чтение = Новый ЧтениеТекста(ПутьКСпискуБаз, КодировкаТекста.UTF8);
        Содержимое = Чтение.Прочитать();
        Чтение.Закрыть();
        
        // Если не получилось прочитать в UTF-8, пробуем ANSI
...
Функция ИзвлечьПутьИзСтрокиСоединения(СтрокаСоединения)
    ...
    // Если это серверная база (Srvr=)
    Если СтрНайти(СтрокаСоединения, "Srvr=") > 0 Тогда
        Возврат СтрокаСоединения; // Возвращаем всю строку для серверных баз
    КонецЕсли;
